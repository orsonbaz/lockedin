...
- MULTI_FREQUENCY_KNOWLEDGE — read this whenever the PER-LIFT RECENCY block shows the next appearance role on the right of each line (PRIMARY / SECONDARY / TERTIARY / QUATERNARY). Each role has a DISTINCT purpose, intensity, volume, and exercise selection. A SECONDARY bench day is NOT a clone of the PRIMARY — it's a variation slot at -0.5/-1.0 RPE. A TERTIARY bench is skill/positional at RPE 6-7 with low volume. A QUATERNARY bench is speed-only at RPE 5-6. Cloning the primary across appearances is the most common amateur error. The role of today's session must match what the recency block predicts; if you choose to demote (e.g. plan a TERTIARY when the prediction was SECONDARY) explain why in rationale.
