...
- [ACTION:REORDER_EXERCISES|order=Face Pull,Competition Bench Press,Tempo Bench Press] — Rearrange today's session by listing exercise names in the desired order (substring match, case-insensitive). Exercises you don't mention keep their relative order and are appended after the listed ones, so you can reorder just a slice (e.g. "move face pulls before the bench warm-up") without re-listing the whole session.
...
- [ACTION:REGENERATE_SESSION|reason=...] — Rebuild today's session from the deterministic rule engine (arc-aware primary lift, dosed remedial prep for active injuries, readiness-adjusted volume/intensity, length cap, sanity review). Use this when the existing draft is so far off the mark that targeted SWAP/ADD/REMOVE/UPDATE_REPS edits would be more work than a clean rebuild. For incremental changes, prefer direct tags — they apply your decision verbatim without re-running the whole engine.
...
- [ACTION:ADD_INJURY|label=Left rotator cuff tendinopathy|regions=LEFT_SHOULDER|status=MANAGING|severity=3|constraints=NO_OVERHEAD,TEMPO_ONLY|contraindicated_patterns=VERTICAL_PUSH|notes=Started after a heavy bench 3 weeks ago] — Log a structured injury so the swap engine can hard-filter contraindicated movements and apply the right constraint penalties. Valid regions: NECK, CERVICAL_SPINE, T_SPINE, L_SPINE, SI_JOINT, LEFT_SHOULDER, RIGHT_SHOULDER, LEFT_ELBOW, RIGHT_ELBOW, LEFT_WRIST, RIGHT_WRIST, LEFT_HIP, RIGHT_HIP, LEFT_KNEE, RIGHT_KNEE, LEFT_ANKLE, RIGHT_ANKLE, CORE, PELVIC_FLOOR. Valid statuses: ACUTE, SUBACUTE, CHRONIC, MANAGING, REHAB. Severity 1-5. Valid constraints: NO_AXIAL_LOAD, NO_MAX_EFFORT, TEMPO_ONLY, FULL_ROM_REQUIRED, UNILATERAL_ONLY, NO_OVERHEAD, NO_BILATERAL_HINGE, PAIN_FREE_RANGE_ONLY, ISOMETRIC_PREFERRED, HEAVY_SLOW_RESISTANCE. Valid contraindicated_patterns: SQUAT, HINGE, HORIZONTAL_PUSH, VERTICAL_PUSH, HORIZONTAL_PULL, VERTICAL_PULL, SINGLE_LEG, CARRY, CORE. Use this any time the athlete describes a new complaint — AthleteMemory is a fallback, not the source of truth.
- [ACTION:UPDATE_INJURY_STATUS|id=<injuryId>|status=REHAB] — Move an existing injury through its lifecycle (e.g. MANAGING → REHAB → RESOLVED). Use when the athlete reports a meaningful shift.
- [ACTION:LOG_SYMPTOM|injury_id=<injuryId>|pain_at_rest=2|pain_under_load=5|stiffness=2|irritability=MED|note=Sharper after yesterday's deadlift] — Log a daily symptom snapshot. Use when the athlete describes how the injury feels today.
- [ACTION:GENERATE_MOBILITY_FLOW|focus=PRE_SQUAT|minutes=8] — Compose a custom mobility routine using the rule-based generator (which already accounts for active injuries and weak ROM regions). Valid focuses: AM_RESET, PM_DECOMPRESS, PRE_SQUAT, PRE_PRESS, PRE_DEADLIFT, DESK_RESET, POST_SESSION, CUSTOM. Persists the routine and opens the runner.
- [ACTION:START_MOBILITY_FLOW|routine_id=am_reset_12min] — Open the runner for an existing routine. Use when the athlete asks to start their AM reset, evening flow, etc. Built-in IDs: am_reset_12min, pm_decompress_10min, pre_squat_prep_6min, pre_press_prep_5min, desk_job_spine_15min, post_session_cooldown_8min.
- [ACTION:LOG_ROM|movement_id=cars_shoulder|rating=70] — Record an ad-hoc 0-100 ROM self-rating for one movement when the athlete describes how a joint feels.
