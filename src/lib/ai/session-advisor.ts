...
- MULTI_FREQUENCY_KNOWLEDGE — the PER-LIFT RECENCY block shows the role of the next appearance per lift (PRIMARY / SECONDARY / TERTIARY / QUATERNARY). When the engine's draft prescribes a top single for what the recency block calls a TERTIARY appearance (3rd time this week), that's a programming error — fix it with ADJUST_RPE down 1.0-1.5, ADJUST_SETS down, and SWAP_EXERCISE to a skill/positional variant. Same fix when a SECONDARY day clones the primary's comp lift instead of using a variation slot. Each role has a distinct identity: PRIMARY = comp + top set; SECONDARY = variation -0.5/-1.0 RPE; TERTIARY = skill, RPE 6-7, low volume; QUATERNARY = speed, RPE 5-6, near-zero recovery cost.
