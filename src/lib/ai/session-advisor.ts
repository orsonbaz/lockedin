...
- MULTI_FREQUENCY_KNOWLEDGE — the PER-LIFT RECENCY block shows the role of the next appearance per lift. Differentiation uses FOUR LEVERS (pick ONE per appearance, don't stack): L1 variation, L2 emphasis rotation, L3 intent shift (speed work), L4 volume / intensity. **Do not reflexively drop RPE per appearance** — Sheiko 3× bench keeps uniform intensity, Calgary 4× bench keeps top sets at 76-82 % across days. Common engine errors to fix: (a) engine prescribed a top single for a TERTIARY appearance → SWAP_EXERCISE to a variation, ADJUST_SETS down (volume cut, NOT auto RPE cut); (b) SECONDARY day clones the primary's comp lift → SWAP_EXERCISE to a variation slot; (c) QUATERNARY day prescribed at full block intensity → either elect speed-work intent (ADJUST_RPE to 5-6, ADJUST_REPS to 3, ADJUST_LOAD to ~55 % 1RM) OR skill-accessory (RPE 6-7) OR remove the day entirely. Roles in shorthand: PRIMARY = comp + top set; SECONDARY = variation slot at near-primary RPE; TERTIARY = volume cut + emphasis change at near-primary RPE; QUATERNARY = intent shift (speed/skill).
...
- REMEDIAL_KNOWLEDGE — when an INJURY memory is active, the athlete's draft session should reflect modern rehab evidence: HEAVY SLOW RESISTANCE for tendons (3-1-3 tempo, ~70 % 1RM), isometrics for acute tendon pain, strengthen-through-range for "tight" muscles BEFORE static stretches, McGill Big 3 for back complaints. If the engine prescribed pure stretching for a complaint that has a load-based modern Rx, swap via ADD_EXERCISE for the loaded variant.
