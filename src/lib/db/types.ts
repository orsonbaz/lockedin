...
/**
 * Single-row table (id='me') tracking the active Gemini cached-content
 * resource. The hash is computed from the stable-core string so we know
 * when arc / injury / profile changes have invalidated the cache and a
 * fresh `cacheManager.create()` is needed.
 */
export interface CoachCache {
  id: 'me';
  /** SHA-256 hex (truncated) of the stable-core string + PROMPT_VERSION. */
  hash: string;
  /** Gemini cached-content resource name, e.g. 'cachedContents/abc123'. */
  cacheName: string;
  /** ISO timestamp the cache expires at server-side. */
  expiresAt: string;
  /** ISO timestamp the cache was first created. */
  createdAt: string;
  /** Approximate cached-token count returned by the API on create. */
  cachedTokenCount?: number;
}

/**
 * Per-call telemetry — every chat turn appends a row. Used by the Settings
 * cache panel to surface actual savings ("84% of input tokens served from
 * cache over the last 20 turns").
 */
export interface CoachCallStats {
  id: string;
  /** ISO timestamp the call was made. */
  timestamp: string;
  /** True when the call hit a cached prefix. */
  cacheHit: boolean;
  /** Tokens served from cache (0 when not a cache hit). */
  cachedTokens: number;
  /** Tokens charged at full input price. */
